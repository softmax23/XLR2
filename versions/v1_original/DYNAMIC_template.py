...
class XLRCreateTemplate(XLRGeneric, XLRSun, XLRTaskScript, XLRControlm, XLRDynamicPhase):
    ...
    def dynamic_phase_dynamic(self) -> str:
        ...
        technical_task = self.dict_value_for_template_technical_task.get('technical_task')
        template_phases = self.dict_value_for_template.get('template_liste_phase', [])
        list_packages = getattr(self, 'list_package', [])

        self.logger_detail.info(f"Dynamic phase check - technical_task: {technical_task is not None}")
        self.logger_detail.info(f"Dynamic phase check - phases count: {len(template_phases)}")
        self.logger_detail.info(f"Dynamic phase check - packages count: {len(list_packages)}")

        if (technical_task is None and
            len(template_phases) == 1 and
            len(list_packages) == 1):
            self.logger_detail.info("Skipping dynamic phase creation - simple template")
            return 'done'

        # Create dynamic_release phase to add Jython tasks
        self.logger_detail.info("Creating dynamic_release phase")
